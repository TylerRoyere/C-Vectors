...
prefix name create_ ## name(const unsigned int capacity) \
{ \
    return (name ) { \
        .vec = create_vector(capacity, sizeof( type )), \
    }; \
} \
prefix name create_ ## name ##_cleared(const unsigned int capacity) \
{ \
    return (name ) { \
...
prefix unsigned int name##_size(name vv) \
{ \
    return vector_size(vv.vec); \
} \
prefix name name##_resize(name vv, const unsigned int new_capacity) \
{ \
    return (name) { \
        .vec = vector_resize(vv.vec, new_capacity), \
    }; \
} \
prefix type name##_get(const name vv, const unsigned int index, int* err) \
{ \
    type ret; \
    NULLABLE_ASSIGNMENT(int, err) = vector_get(vv.vec, index, &ret); \
    return ret; \
} \
prefix int name##_set(name vv, const unsigned int index, type val) \
{ \
    return vector_set(vv.vec, index, &val); \
} \
prefix type* name##_get_ref(name vv, const unsigned int index, int* err) \
{ \
    type* ret = NULL; \
...
name create_ ## name (const unsigned int capacity); \
name create_ ## name ## _cleared(const unsigned int capacity); \
name name##_fill(name vv); \
name name##_fill_value(name vv, type fill); \
int destroy_ ## name (name vv); \
unsigned int name##_size(name vv); \
name name##_resize(name vv, const unsigned int new_capacity); \
type name##_get(const name vv, const unsigned int index, int* err); \
int name##_set(name vv, const unsigned int index, type val); \
type* name##_get_ref(name vv, const unsigned int index, int* err); \
